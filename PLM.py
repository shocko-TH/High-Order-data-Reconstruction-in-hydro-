...
#-----------------------------------------------------------
# PLM with van Leer limiter + Local Lax-Friedrichs (Rusanov)
#----------------------------------------------------------
...
def Update( U, dt ):
    ...
    # U後 = U前 - 流進流出量
    # update real cells
    U[nghost:Nx-nghost,nghost:Ny-nghost,:] -= dt/dx*( flux_x[nghost+1:Nx-nghost+1, nghost:Ny-nghost, :]
                                                     -flux_x[nghost  :Nx-nghost,   nghost:Ny-nghost, :] )

    U[nghost:Nx-nghost,nghost:Ny-nghost,:] -= dt/dy*( flux_y[nghost:Nx-nghost, nghost+1:Ny-nghost+1, :]
                                                     -flux_y[nghost:Nx-nghost, nghost  :Ny-nghost,   :] )

    # 確定沒有負壓負密度
    # keep density and pressure positive in a simple way
    W = Conserved2Primitive( U )
